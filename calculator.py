...
def updateDisplay(char):
    ...
    if not char.isalnum() and char != "." and g_leftOperand:
        ...
        g_leftOperand = str(g_leftOperand) + str(char)
    elif (char.isalnum() or char==".") and not g_leftOpFlag:
        g_rightOperand = str(g_rightOperand) + str(char)
...
def finishDisplay(OpCheck):
    ...
    for index, key in enumerate(str(g_leftOperand)):
        if key == "pi":
            g_leftOperand = g_leftOperand.replace("pi", "", 1)
            if g_leftOperand:
    ...
    for index,key in enumerate(str(g_rightOperand)):
        if key == "pi":
            g_rightOperand = g_rightOperand.replace("pi", "", 1)
            if g_rightOperand:
...
def roundDisplay():
    global g_leftOperand
    global entry
    entry = str(entryRound.get())

    if entry == "":
        entry = 1

    g_leftOperand = round(float(g_leftOperand), int(entry))
    labelOutput.configure(text=g_leftOperand)
...
buttonPi = Button(root, text="pi", command=lambda: updateDisplay("pi"))
